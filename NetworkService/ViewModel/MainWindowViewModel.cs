...
using NetworkService.ComboBoxImages;
...
namespace NetworkService.ViewModel
{
    public class MainWindowViewModel : BindableBase
    {
        ...
        private void OnCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (DER newEntity in e.NewItems)
                {
            ...
            if (e.OldItems != null)
            {
                foreach (DER oldEntity in e.OldItems)
                {
                    ...
                    {
                        int canvasIndex = networkDisplayViewModel.GetCanvasIndexForDERId(oldEntity.Id);
                        networkDisplayViewModel.DeleteDERFromCanvas(oldEntity);
                    }
                }
            }
        }
        ...
        {
            var tcp = new TcpListener(IPAddress.Any, 25665);
            tcp.Start();
                        ...
                        else
                        {
                            ...
                            for (int idx = 0; idx < networkEntitiesViewModel.Entities.Count; idx++)
                            {
                                ...
                                if (currentEntityId == incommingEntityId)
                                {
                                    ...
                                    networkDisplayViewModel.UpdateDEROnCanvas(networkEntitiesViewModel.Entities[idx]);
